 ...
 * represents server that is responsible for communicating with clients and
...
public:
  ...
  /**
   * @brief registerNewClient attempts to register client and add him into map,
   * returns true if insertion into map was successful, false otherwise
   * @param clientName name of the new client
   * @param connection pointer to connection on which client communicates with
   * the server
   * @return
   */
  ...
  /**
   * @brief sendConnectionAccept called on server by accepting client to send
   * request accept to the sender
...
public slots:
  ...
  /**
   * @brief sendConnectionRequest method called by client to initialize
