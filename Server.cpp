...
void Server::incomingConnection(qintptr socketDescriptor) {
  ...
  connect(connection, SIGNAL(onRegistrationRequest(ClientInfo)), this,
          SLOT(processRegistrationRequest(ClientInfo)));
  connect(connection, SIGNAL(onCreateChannelRequest(QString, QJsonObject)),
          this, SLOT(sendChannelRequest(QString, QJsonObject)));
  connect(connection, SIGNAL(onCreateChannelReply(QString, QJsonObject)), this,
          SLOT(sendChannelReply(QString, QJsonObject)));
  connect(connection, SIGNAL(onQuit(Connection *)), this,
          SLOT(removeClient(Connection *)));
...
void Server::processRegistrationRequest(ClientInfo clInfo) {
  // qDebug() << "process registration request on server was called name: " <<
  // name;
